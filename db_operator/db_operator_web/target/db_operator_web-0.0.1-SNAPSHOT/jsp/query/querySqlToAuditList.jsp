...
<script type="text/javascript">
	function changePage(n){
		...
		$.ajax({
            type: 'POST',
            url: "<%=path%>/auditConsole/refreshToAuditSqls.htm",
					data : {
						pageNum : pageNum,
						pageSize : pageSize,
						excReasons : $("#excReasons").val().trim(),
						excStaus : $("#excStaus").val().trim(),
						excDataSourId : $("#sid").val(),
						excOper : $("#excOper").val().trim(),
	...
	function openChildInfo(tr) {
		...
		window.open ('<%=path%>/DBConsole/getOneSqlInfo.htm?querySqlId='+querySqlId+'&ifAuditPage='+'0'+'&pageNum=1&pageSize=10','oneSqlInfo','height=750,width=1000,top=100,left=200,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no') 
	}
</script>
	...
	<form action=" " method="post" id="querySqlListForm"
		...
		<div class="con_search">
			<table width="100%" border="0" cellpadding="0" cellspacing="10px">
				<tr>
					<td class="con_tableBg_left" width="20%"><span>当前的位置:sql语义待审核</span>
					</td>
				</tr>
			</table>
		</div>
		...
		<div class="con_search">
			<table width="100%" border="0" cellpadding="0" cellspacing="10px">
				...
				<tr>
					...
					<td class="con_search_right">提交人：</td>
					<td>
						<input type="text" class="inputStyle" id="excOper" name="excOper" size="30">
					</td>
				</tr>
				<tr>
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="0"
				...
				<c:forEach items="${objs}" var="obj" varStatus="s">
					<tr class="tableConBg" ondblclick="openChildInfo(this);">
						...
						<td>${obj.submitObj.opernum }</td>
						<td>${obj.ts }</td>
						<td>
							<c:if test="${obj.auditStatus == '1' }">已提交</c:if>
							<c:if test="${obj.auditStatus == '0' }">已通过</c:if>
							<c:if test="${obj.auditStatus == '2' }">未通过</c:if>
							<c:if test="${obj.auditStatus == '3' }">已执行</c:if>
						</td>
					</tr>
				</c:forEach>
