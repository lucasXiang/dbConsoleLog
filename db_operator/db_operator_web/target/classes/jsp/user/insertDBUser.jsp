...
<script type="text/javascript">
	function addOneUser(){
		...
		var funParam=document.getElementsByName("funParam");
		for(i in funParam){
			if(funParam[i].value==85 && !funParam[i].checked){
				var funcMenu=document.getElementsByName("funcMenus");
				for(i in funcMenu){
					if(funcMenu[i].checked){
						alert("请先选择 模板列表 权限！");
						return;
					}
				}
		...
		if(confirm("确定要提交用户信息吗？")){
			...
			var dbSours=document.getElementsByName("dbSours");
			var dbSoursArr=[];
			for(i in dbSours){
				if(dbSours[i].checked){
					dbSoursArr.push(dbSours[i].value);
				}
			}
			var funcMenus=document.getElementsByName("funcMenus");
			var funcMenusArr=[];
			for(i in funcMenus){
				if(funcMenus[i].checked){
					funcMenusArr.push(funcMenus[i].value);
				}
			}
			$.ajax({
				type:'POST',
				...
				data:{
					...
					dbSoursStr:dbSoursArr.join(','),
					funcMenusStr:funcMenusArr.join(',')
				},
				dataType:'json',
	...
	function reset(){
		...
		for(i in dbSours){
			if(dbSours[i].checked){
				dbSours[i].checked=false;
			}
		}
		var funcMenus=document.getElementsByName("funcMenus");
		for(i in funcMenus){
			if(funcMenus[i].checked){
				funcMenus[i].checked=false;
			}
		}
	}
</script>
		...
		<div class="con_search" id="excDiv">
			<table width="100%" border="0" cellpadding="0" cellspacing="10px">
					...
					<td colspan="4">
						<c:forEach var="par" items="${parFuns}" >
							...
							<c:forEach var="subFun" items="${par.chFuncObjs }" varStatus="s">
								...
								<c:if test="${subFun.id == 85 }">
									<br>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;功能菜单：
									<label><input type="checkbox" name="funcMenus" value="0">新增</label>
									<label><input type="checkbox" name="funcMenus" value="1">修改</label>
									<label><input type="checkbox" name="funcMenus" value="2">删除</label>
								</c:if>
				...
				<tr>
					<td colspan="4">
						&nbsp;&nbsp;&nbsp;&nbsp;
						<c:forEach var="db" items="${dbSours }">
							<label><input type="checkbox" name="dbSours" value="${db.id }">${db.sid }</label>
							<c:if test="${(s.index+1)%5 == 0 }"><br>&nbsp;&nbsp;&nbsp;&nbsp;</c:if>
						</c:forEach>
					</td>
				</tr>
